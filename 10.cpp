    ...
    auto _execute(int &x, function<void()> tick) {
        auto F = [&](const string &val) {
            if (val == NOOP) {
                tick();
                return;
            } else if (auto temp{split(val, regex{"\\ +"})};
                    temp[0] == ADDX) {
...
public:
    ...
    TYPE earlyState() {
        ...
        const array<int, 6> positions{
            20, 60, 100, 140, 180, 220
        };

        const auto sum = reduce(positions.begin(),
                                positions.end(),int64_t{0},
                                [&cycles](
                                        auto &s, const auto &pos
                                        ) {
    ...
    auto succeedingState() -> vector<string> {
        ...
        auto tick = [&crt, &pos, &x]() {
            if (x - 1 <= (pos % WIDTH) && (pos % WIDTH) <= x + 1) {
                crt[pos] = LIT;
            }
            pos++;
        };
...
int main() {
    auto main = DayTen("../inputs/dayTen");
    main
      .cacheRes(main
      .earlyState()),
    main.firstFragment(),
